...
now uses Pathom for server-side mutation and query handling.  See `clojure_fullschtack/server_components/pathom_wrappers.clj` and
`clojure_fullschtack/model/user.clj` for the macro definitions and examples of how to define
...
`src/main/clojure_fullschtack/client.cljs` contains the code to start and refresh
...
See the server section below for working on the full-stack clojure_fullschtack itself.
...
application `clojure_fullschtack.development-preload`. You can add code here that
...
```
lein uberjar
java -jar target/clojure_fullschtack.jar
```
